#!/usr/bin/env python3
"""Merge SKU 49674 into PA Bluestone Slate as a new variant.
Recreates the product with 4 variants (adding 24"x24"x1 1/2" after 18"x24").
Deletes the standalone 49674 product.
...
OLD_PRODUCT_IDS = [
    "gid://shopify/Product/10432040108324",  # PA Bluestone Slate (current 3-variant product)
    "gid://shopify/Product/10409669263652",  # Blue Stone 24 X 24 X 1.5 (standalone SKU 49674)
]

DESCRIPTION_HTML = """<p>Set a clean, modern surface with PA Bluestone Slate. These thermally finished pieces deliver a smooth, consistent texture with the classic blue-grey tones of Pennsylvania bluestone. Buy individual pieces in four sizes to build patios, walkways, landings, and pool surrounds with precise, tight-joint installations.</p>
...
<ul>
  ...
  <li><strong>24&rdquo; &times; 24&rdquo; &times; 1 1/2&rdquo;:</strong> Square-format pieces for grid patterns, landings, and focal areas.</li>
...
<ul>
  ...
  <li><strong>Classic bluestone color:</strong> Natural blue-grey tones that complement traditional and contemporary landscapes.</li>
...
<h3>Applications</h3>
<p>Build formal patios, entry landings, and pool surrounds with a refined, timeless look. Set walkways with uniform joints or combine sizes for running bond and basketweave patterns. Use for step treads, coping, and transitions between indoor and outdoor living spaces.</p>
...
<ul>
  ...
  <li><strong>Sizes:</strong> 12&rdquo;&times;24&rdquo;, 18&rdquo;&times;24&rdquo;, 24&rdquo;&times;24&rdquo;, 18&rdquo;&times;36&rdquo;</li>
</ul>
...
PRODUCT_INPUT = {
    "title": "PA Bluestone Slate",
    "handle": "pa-bluestone-slate",
    "vendor": "Everlast",
    ...
    "productOptions": [
        {"name": "Size", "position": 1, "values": [
            {"name": '12"x24"x1 1/2"'}, {"name": '18"x24"x1 1/2"'},
            {"name": '24"x24"x1 1/2"'}, {"name": '18"x36"x1 1/2"'},
        ]},
    ...
    "variants": [
        {
            ...
            "price": "24.99", "sku": "41506",
            "inventoryPolicy": "DENY",
            "inventoryItem": {"tracked": True, "sku": "41506", "cost": "19.70", "measurement": {"weight": {"value": 45.0, "unit": "POUNDS"}}},
            "inventoryQuantities": [{"locationId": LOCATION_ID, "name": "available", "quantity": 74}],
        },
        {
            "optionValues": [
            ...
            "price": "28.00", "sku": "41505",
            "inventoryPolicy": "DENY",
            "inventoryItem": {"tracked": True, "sku": "41505", "cost": "22.25", "measurement": {"weight": {"value": 62.0, "unit": "POUNDS"}}},
            "inventoryQuantities": [{"locationId": LOCATION_ID, "name": "available", "quantity": 0}],
        },
        {
            "optionValues": [
                {"optionName": "Size", "name": '24"x24"x1 1/2"'},
                {"optionName": "Type", "name": "Thermal"},
                {"optionName": "Unit of Sale", "name": "Per Piece"},
            ],
            "price": "51.99", "sku": "49674",
            "inventoryPolicy": "DENY",
            "inventoryItem": {"tracked": True, "sku": "49674", "cost": "36.03", "measurement": {"weight": {"value": 91.0, "unit": "POUNDS"}}},
            "inventoryQuantities": [{"locationId": LOCATION_ID, "name": "available", "quantity": 20}],
        },
        {
            "optionValues": [
            ...
            "price": "49.00", "sku": "41507",
            "inventoryPolicy": "DENY",
            "inventoryItem": {"tracked": True, "sku": "41507", "cost": "39.96", "measurement": {"weight": {"value": 93.0, "unit": "POUNDS"}}},
            "inventoryQuantities": [{"locationId": LOCATION_ID, "name": "available", "quantity": 0}],
        },
    ],
}
...
def gql(cfg, q, v=None):
    ...
    r.raise_for_status()
    d = r.json()
    if "errors" in d: raise RuntimeError(f"GraphQL errors: {d['errors']}")
    return d["data"]
...
def main():
    ...
    log.info(f"Merge 49674 into PA Bluestone Slate — mode: {mode}")
    ...
    for pid in OLD_PRODUCT_IDS:
        print(f"     {pid}")
    print(f"2. Create product '{PRODUCT_INPUT['title']}' with 4 variants:")
    for v in PRODUCT_INPUT["variants"]:
        opts = " / ".join(o["name"] for o in v["optionValues"])
        qty = v["inventoryQuantities"][0]["quantity"]
        cost = v["inventoryItem"].get("cost", "N/A")
        wt = v["inventoryItem"]["measurement"]["weight"]["value"]
        print(f"     SKU {v['sku']:<7} {opts:<50} ${v['price']:<8} cost=${str(cost):<7} wt={wt:<6} inv={qty}")
    ...
    log.info("Creating 4-variant product via productSet...")
