#!/usr/bin/env python3
"""Upload 1 processed PNG for Everlast 'Laurel Mountain Slabs'.

Product (queried at design time):
  handle = laurel-mountain-slabs
  gid    = gid://shopify/Product/10432064684324
  vendor = Everlast
  options:
    position 1 = 'Size'         values = ['4"-6"']               (degenerate: 1 value)
    position 2 = 'Unit of Sale' values = ['Per Pallet', 'Per LB'] (real differentiator)
  variants (2):
    SKU 36949  title='4"-6" / Per Pallet'
    SKU 36950  title='4"-6" / Per LB'
  media: EMPTY (no images uploaded yet)

Alt-tag strategy:
  We have ONE image (a pallet product shot) that should be shown on BOTH
  variants regardless of Unit of Sale selection. Using a hashtag like
  '#Per Pallet' would hide the image for the 'Per LB' variant; using no
  hashtag lets the theme filter skip filtering and keep the image visible
  for every variant. The theme filter JS (product-gallery.liquid:297-313)
  leaves an alt-without-'#' image visible on every variant.

  Alt = "Laurel Mountain Slabs"

Pipeline:
  1. Query product by handle, verify structure matches the design-time snapshot
     (2 variants, SKUs 36949/36950, Size degenerate, Unit of Sale with 2 values,
     0 media). If unexpected, STOP and report.
  2. Stage + upload bytes + productCreateMedia with the chosen alt.
     Poll until READY, capture MediaImage gid + CDN url.
  3. productVariantAppendMedia - link the single image to BOTH variant gids.
     Handle the 'variant already has attached media' userError as a soft warn
     (idempotent re-run safety, same pattern as Pro-Lime).
  4. metafieldsSet - custom.color_swatch_image = CDN URL for each variant.
  5. Verification:
       - Re-query: confirm 1 MediaImage, alt correct, both variants have
         image.url and metafield set
       - Python theme-filter simulation: both variants should see the image
       - curl live page with cache-buster, confirm CDN URL present and
         the image is NOT suppressed with 'is-filtered'

Usage:
  python3 upload_laurel_mountain_image.py --dry-run
  python3 upload_laurel_mountain_image.py          # executes
"""
...
PRODUCT_HANDLE = "laurel-mountain-slabs"
...
IMAGE_PATH = "/tmp/garoppos_product_images/laurel_mountain_slab_processed.png"

# Alt text strategy: descriptive, NO '#' so the theme filter keeps the image
# visible on all variants. Size is degenerate (single value), and the one
# image covers both Unit of Sale variants.
IMAGE_ALT = "Laurel Mountain Slabs"

EXPECTED_VARIANT_SKUS = {"36949", "36950"}
EXPECTED_VARIANT_COUNT = 2
EXPECTED_OPTION_NAMES = ["Size", "Unit of Sale"]
EXPECTED_SIZE_VALUES = ['4"-6"']
...
def validate_structure(product):
    """Abort if product deviates from the expected Laurel Mountain shape.

    Expected: 2 options (Size degenerate at '4"-6"', Unit of Sale with
    'Per Pallet' and 'Per LB'), 2 variants (SKUs 36949 and 36950), no
    existing media.
    """
    ...
    if list(size_opt.get("values") or []) != EXPECTED_SIZE_VALUES:
        sys.exit(
            f"STOP: Size option values changed. Expected {EXPECTED_SIZE_VALUES}, got {size_opt.get('values')}. "
            "Re-evaluate alt-tag strategy (may need hashtag on Size)."
        )

    uos_opt = options_sorted[1]
    uos_values = set(uos_opt.get("values") or [])
    if uos_values != EXPECTED_UOS_VALUES_SET:
        sys.exit(
            f"STOP: Unit of Sale values changed. Expected {EXPECTED_UOS_VALUES_SET}, got {uos_values}. "
...
def main():
    ...
    print("  value = image CDN URL (same URL for both variants)")
    print("-" * 60)
