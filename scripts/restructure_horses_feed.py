"""
Restructure Horses subcategories in Shopify.

Changes:
1. Fetch all products from "Horses Feed" and "Horses Feed (includes hay)" collections
2. Classify each product as TREAT or FEED/HAY based on title keywords
3. Create new "Horses Treats" collection
4. Retag all products (remove old tags, add new ones)
5. Rename "Horses Feed" collection → "Horses Feed and Hay"
6. Delete "Horses Feed (includes hay)" collection
...
Usage:
    ...
    python scripts/restructure_horses_feed.py
"""
...
HORSES_FEED_GID = 'gid://shopify/Collection/651861885220'
HORSES_FEED_HAY_GID = 'gid://shopify/Collection/651582570788'
...
def phase1_collect():
    ...
    logger.info(f"Fetching Horses Feed ({HORSES_FEED_GID})...")
    feed_products = fetch_collection_products(HORSES_FEED_GID)

    logger.info(f"Fetching Horses Feed (includes hay) ({HORSES_FEED_HAY_GID})...")
    hay_products = fetch_collection_products(HORSES_FEED_HAY_GID)
...
# ─── Phase 3: Create "Horses Treats" collection ───────────────────────────────

def phase3_create_treats_collection() -> dict:
    logger.info("=" * 60)
    logger.info("PHASE 3: Creating 'Horses Treats' collection")
    logger.info("=" * 60)

    rules = [
        {'column': 'TAG', 'relation': 'EQUALS', 'condition': 'Horses'},
    ...
    result = create_collection(
        ...
        rules,
        cfg,
        description='Shop horse treats including cookies, nibbles, and flavored snacks.'
    )
    if not result:
        raise RuntimeError("Failed to create 'Horses Treats' collection")
...
# ─── Phase 5: Rename "Horses Feed" → "Horses Feed and Hay" ───────────────────

def phase5_rename_collection():
    logger.info("=" * 60)
    logger.info("PHASE 5: Renaming 'Horses Feed' → 'Horses Feed and Hay'")
    logger.info("=" * 60)
...
# ─── Phase 6: Delete "Horses Feed (includes hay)" collection ──────────────────

def phase6_delete_old_collection():
    logger.info("=" * 60)
    logger.info("PHASE 6: Deleting 'Horses Feed (includes hay)' collection")
    logger.info("=" * 60)

    mutation = """
    mutation {
      collectionDelete(input: {id: "gid://shopify/Collection/651582570788"}) {
        deletedCollectionId
        userErrors { field message }
      }
    }
...
def phase7_update_menu(treats_collection: dict):
    ...
    # Find "Livestock and Farm" → "Horses"
    livestock_item = None
    ...
    for item in livestock_item.get('items', []):
        if 'horses' in item['title'].lower():
            horses_item = item
            break
    if not horses_item:
        for item in livestock_item.get('items', []):
            logger.info(f"  Livestock sub-item: {item['title']}")
        raise RuntimeError("Could not find 'Horses' under Livestock and Farm")

    logger.info(f"Found Horses item: '{horses_item['title']}'")

    # Inspect Horses sub-items
    horses_subitems = horses_item.get('items', [])
    logger.info(f"Current Horses sub-items ({len(horses_subitems)}):")
    for sub in horses_subitems:
        logger.info(f"  - {sub['title']} (type={sub.get('type')}, resourceId={sub.get('resourceId')}, url={sub.get('url')})")

    # Build new horses sub-items:
    # 1. Rename "Feed" → "Feed and Hay" (keep same id/resourceId)
    # 2. Remove "Feed (includes hay)"
    # 3. Add "Treats" after Feed and Hay
    # 4. Keep all others

    feed_and_hay_item = None
    new_horses_subitems = []
    ...
    if not treats_inserted:
        ...
        new_horses_subitems.insert(0, treats_item)

    logger.info(f"\nNew Horses sub-items ({len(new_horses_subitems)}):")
    for sub in new_horses_subitems:
        logger.info(f"  - {sub['title']}")

    # Rebuild the full menu tree with the updated Horses subitems
    def rebuild_items(items, target_horses_title, new_subitems):
        """Recursively rebuild menu items, replacing horses subitems."""
        rebuilt = []
        for item in items:
            if item['title'].lower() == target_horses_title.lower():
                updated = dict(item)
                updated['items'] = new_subitems
                rebuilt.append(updated)
            elif item.get('items'):
                updated = dict(item)
                updated['items'] = rebuild_items(item['items'], target_horses_title, new_subitems)
                rebuilt.append(updated)
            else:
                rebuilt.append(item)
        return rebuilt

    updated_top_items = rebuild_items(top_items, horses_item['title'], new_horses_subitems)
    ...
    if menu_after:
        # Find horses sub-items in updated menu
        for top in menu_after['items']:
            for sub in top.get('items', []):
                if 'horses' in sub['title'].lower():
                    logger.info(f"\nVerification — Horses sub-items in updated menu:")
...
def report(products, classifications, treats_collection, renamed_coll):
    ...
    logger.info(f"Treats retagged and moved to 'Horses Treats':  {treat_count}")
    logger.info(f"Feed/Hay retagged for 'Horses Feed and Hay':   {feed_hay_count}")
    logger.info(f"'Horses Feed and Hay' collection id:           {HORSES_FEED_GID}")
    logger.info(f"  Products in renamed collection:              {renamed_coll.get('productsCount', {}).get('count', '?')}")
    logger.info(f"'Horses Treats' collection id:                 {treats_collection['id']}")
    logger.info(f"'Horses Treats' collection handle:             {treats_collection['handle']}")
    logger.info(f"(Note: collection product counts reflect tag changes — may take Shopify a moment to sync)")

    # Show final Horses menu state
    invalidate_menu_cache()
    menu = get_menu_by_handle('main-menu', cfg)
    if menu:
        for top in menu['items']:
            for sub in top.get('items', []):
                if 'horses' in sub['title'].lower():
                    logger.info(f"\nHorses subcategories in main menu (in order):")
                    for s in sub.get('items', []):
...
def main():
    logger.info("Starting Horses feed restructure...")
    ...
    # Phase 3: Create Treats collection
