    ...
    try:
        ...
        while page_count < max_pages:
            ...
            query searchTaxonomy($cursor: String) {
              taxonomyCategories(first: 250, after: $cursor) {
                edges {
                  node {
                    id
                    fullName
            ...
            taxonomy_data = result.get("data", {}).get("taxonomyCategories", {})
        ...
        for edge in edges:
            ...
            if full_name_lower == category_lower:
                exact_match = node
                break

            # Check if the search term is contained in the fullName
            if category_lower in full_name_lower:
                if not best_match:
                    best_match = node

        # Use exact match first, then best match
        match = exact_match or best_match

        if match:
            taxonomy_id = match.get("id")
            full_name = match.get("fullName")
            match_type = "exact" if exact_match else "partial"
            if status_fn:
                log_and_status(status_fn, f"  Found taxonomy {match_type} match: {full_name} -> {taxonomy_id}")
            else:
                logging.info(f"  Found taxonomy {match_type} match: {full_name} -> {taxonomy_id}")
            return taxonomy_id

        if status_fn:
            log_and_status(status_fn, f"  No taxonomy match found for: {category_name}")
        else:
            logging.info(f"  No taxonomy match found for: {category_name}")
        return None
...
def get_taxonomy_id(category_name, taxonomy_cache, api_url, headers, status_fn=None):
    ...
    if status_fn:
        log_and_status(status_fn, f"  Searching Shopify taxonomy for: {category_name}")
    else:
        logging.info(f"  Searching Shopify taxonomy for: {category_name}")
    ...
    if taxonomy_id:
        ...
        if status_fn:
            log_and_status(status_fn, f"  Cached new taxonomy mapping: {category_name} -> {taxonomy_id}")
        else:
            logging.info(f"  Cached new taxonomy mapping: {category_name} -> {taxonomy_id}")
    else:
        # Cache the failure to avoid repeated lookups
        taxonomy_cache[category_name] = None
        save_taxonomy_cache(taxonomy_cache)
        if status_fn:
            log_and_status(status_fn, f"  Could not find taxonomy ID for: {category_name}", "warning")
        else:
            logging.warning(f"  Could not find taxonomy ID for: {category_name}")
