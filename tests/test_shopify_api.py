...
# ============================================================================
# GITHUB TAXONOMY FETCH TESTS (Simplified)
# ============================================================================
...
class TestFetchShopifyTaxonomyFromGithub:
    ...
    @patch('uploader_modules.shopify_api.requests.get')
    def test_fetch_from_github_success(self, mock_get, mock_exists):
        """Test successful fetch from GitHub."""
        # Mock cache doesn't exist, force GitHub fetch
        mock_exists.return_value = False

        # Mock GitHub response
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.text = """gid://shopify/TaxonomyCategory/aa-1-2-3 : Animals & Pet Supplies > Pet Supplies > Dog Supplies
gid://shopify/TaxonomyCategory/bb-2-3-4 : Apparel & Accessories > Clothing
gid://shopify/TaxonomyCategory/cc-3-4-5 : Home & Garden"""
        mock_get.return_value = mock_response

        result = shopify_api.fetch_shopify_taxonomy_from_github()

        assert len(result) == 3
        assert result[0]['id'] == 'gid://shopify/TaxonomyCategory/aa-1-2-3'
        assert 'Dog Supplies' in result[0]['fullName']

    @patch('os.path.exists')
    @patch('uploader_modules.shopify_api.requests.get')
    def test_fetch_github_network_error(self, mock_get, mock_exists, caplog):
    ...
    @patch('os.path.exists')
    @patch('uploader_modules.shopify_api.requests.get')
    def test_fetch_github_parse_invalid_lines(self, mock_get, mock_exists):
        """Test parsing skips invalid format lines."""
        # Mock cache doesn't exist, force GitHub fetch
        mock_exists.return_value = False

        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.text = """gid://shopify/TaxonomyCategory/valid1 : Valid Category 1
invalid line without colon
not:a:valid:gid : Some Category
gid://shopify/TaxonomyCategory/valid2 : Valid Category 2
"""
        mock_get.return_value = mock_response

        result = shopify_api.fetch_shopify_taxonomy_from_github()

        # Should only include valid lines with proper GID format
        assert len(result) == 2
        assert result[0]['id'] == 'gid://shopify/TaxonomyCategory/valid1'
        assert result[1]['id'] == 'gid://shopify/TaxonomyCategory/valid2'

    @patch('os.path.exists')
    @patch('uploader_modules.shopify_api.requests.get')
    def test_fetch_github_with_status_fn(self, mock_get, mock_exists):
        """Test fetch with status function."""
        # Mock cache doesn't exist, force GitHub fetch
        mock_exists.return_value = False

        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.text = "gid://shopify/TaxonomyCategory/test : Test Category"
        mock_get.return_value = mock_response

        mock_status_fn = Mock()

        result = shopify_api.fetch_shopify_taxonomy_from_github(status_fn=mock_status_fn)

        assert len(result) == 1
        # Status function should have been called
        assert mock_status_fn.call_count >= 1

    @patch('builtins.open', create=True)
    @patch('os.path.exists')
    def test_fetch_with_valid_cache(self, mock_exists, mock_open):
        """Test using valid recent cache."""
        ...
        # Mock cache file data (recent cache)
        cached_time = datetime.now() - timedelta(days=5)
        cache_data = {
        ...
        # Need to make json.load work with our mock
        import io
        mock_open.return_value.__enter__.return_value = io.StringIO(json.dumps(cache_data))

        result = shopify_api.fetch_shopify_taxonomy_from_github()

        # Should return cached data
        assert len(result) == 1
        assert result[0]['id'] == 'gid://shopify/TaxonomyCategory/test'

    @patch('builtins.open', create=True)
    @patch('os.path.exists')
    def test_fetch_with_valid_cache_and_status_fn(self, mock_exists, mock_open):
        """Test using valid cache with status_fn."""
        from datetime import datetime, timedelta
        import json
    ...
    @patch('builtins.open', create=True)
    @patch('os.path.exists')
    def test_fetch_with_expired_cache(self, mock_exists, mock_open, mock_get):
        """Test fetching fresh data when cache is expired."""
        from datetime import datetime, timedelta
        import json
        ...
        # Mock cache file data (old cache - 40 days)
        cached_time = datetime.now() - timedelta(days=40)
        cache_data = {
            'cached_at': cached_time.isoformat(),
            'categories': [
                {'id': 'gid://shopify/TaxonomyCategory/old', 'fullName': 'Old Category'}
            ]
        }
    ...
    @patch('builtins.open', create=True)
    @patch('os.path.exists')
    def test_fetch_with_corrupted_cache(self, mock_exists, mock_open, mock_get, caplog):
        ...
        assert len(result) == 1
        assert "Failed to load taxonomy cache" in caplog.text

    @patch('uploader_modules.shopify_api.requests.get')
    @patch('builtins.open', create=True)
    @patch('os.path.exists')
    def test_fetch_with_empty_cached_categories(self, mock_exists, mock_open, mock_get):
        """Test handling cache with empty categories list."""
        from datetime import datetime, timedelta
        import json
        ...
        # Mock cache with empty categories
        cached_time = datetime.now() - timedelta(days=5)
        cache_data = {
            'cached_at': cached_time.isoformat(),
        ...
        # Should fetch fresh data since cache is empty
        assert len(result) == 1

    @patch('uploader_modules.shopify_api.requests.get')
    @patch('os.path.exists')
    def test_fetch_with_malformed_lines(self, mock_exists, mock_get):
        """Test skipping malformed lines to cover line 1192."""
        # Mock cache doesn't exist
        mock_exists.return_value = False

        # Mock GitHub response with malformed lines
        mock_response = Mock()
        mock_response.status_code = 200
        # Line with multiple ' : ' should be skipped (line 1192)
        mock_response.text = """gid://shopify/TaxonomyCategory/valid : Valid Category
malformed line with : too : many : separators
gid://shopify/TaxonomyCategory/valid2 : Valid Category 2"""
        mock_get.return_value = mock_response
    ...
    @patch('uploader_modules.shopify_api.requests.get')
    @patch('os.path.exists')
    def test_fetch_cache_write_failure(self, mock_exists, mock_get, mock_open, caplog):
        """Test cache write failure to cover lines 1219-1220."""
        import logging
        ...
        # Mock open to fail on write (when creating cache file)
        mock_open.side_effect = PermissionError("Cannot write cache")

        with caplog.at_level(logging.WARNING):
            result = shopify_api.fetch_shopify_taxonomy_from_github()

        # Should still return categories even if cache write fails
        assert len(result) == 1
        assert "Failed to cache taxonomy" in caplog.text

    @patch('uploader_modules.shopify_api.requests.get')
    @patch('os.path.exists')
    def test_fetch_failure_with_status_fn(self, mock_exists, mock_get):
        """Test fetch failure with status_fn to cover line 1226."""
        import requests

        # Mock cache doesn't exist
        mock_exists.return_value = False

        # Mock GitHub request to fail
        mock_get.side_effect = requests.exceptions.ConnectionError("Network error")
    ...
    @patch('uploader_modules.shopify_api.requests.get')
    @patch('os.path.exists')
    def test_fetch_failure_fallback_to_stale_cache(self, mock_exists, mock_get, mock_open, caplog):
        """Test fallback to stale cache when fetch fails to cover lines 1232-1240."""
        import requests
        import json
        import logging
        from datetime import datetime, timedelta

        # Mock cache doesn't exist initially (first check), but exists for fallback (second check)
        mock_exists.side_effect = [False, True]  # First check: no cache, second check: has stale cache

        # Mock GitHub request to fail
        mock_get.side_effect = requests.exceptions.ConnectionError("Network error")

        # Mock stale cache file
        cached_time = datetime.now() - timedelta(days=50)  # Very old cache
        cache_data = {
            'cached_at': cached_time.isoformat(),
            'categories': [
                {'id': 'gid://shopify/TaxonomyCategory/stale', 'fullName': 'Stale Category'}
            ]
        }
        ...
        assert len(result) == 1
        assert result[0]['id'] == 'gid://shopify/TaxonomyCategory/stale'
        assert "Using stale cache as fallback" in caplog.text


# ============================================================================
# FETCH ALL CATEGORIES TESTS
# ============================================================================
...
class TestFetchAllShopifyCategories:
    ...
    def test_fetch_single_page(self, mock_post):
        """Test fetching categories from single page."""
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "data": {
                "taxonomy": {
                    "categories": {
                        "edges": [
                            {"node": {"id": "gid://shopify/TaxonomyCategory/1", "fullName": "Category 1"}},
                            {"node": {"id": "gid://shopify/TaxonomyCategory/2", "fullName": "Category 2"}}
                        ],
                        "pageInfo": {"hasNextPage": False}
                    }
                }
        ...
        result = shopify_api.fetch_all_shopify_categories(api_url, headers)
        
        assert len(result) == 2
        assert result[0]['id'] == 'gid://shopify/TaxonomyCategory/1'
        assert result[1]['fullName'] == 'Category 2'
        assert mock_post.call_count == 1

    @patch('uploader_modules.shopify_api.requests.post')
    def test_fetch_multiple_pages(self, mock_post):
        """Test fetching categories across multiple pages."""
        # First page
        mock_response_page1 = Mock()
        mock_response_page1.status_code = 200
        mock_response_page1.json.return_value = {
        ...
        assert len(result) == 2
        assert result[0]['id'] == 'gid://shopify/TaxonomyCategory/1'
        assert result[1]['id'] == 'gid://shopify/TaxonomyCategory/2'
        assert mock_post.call_count == 2

    @patch('uploader_modules.shopify_api.requests.post')
    def test_fetch_graphql_errors(self, mock_post, caplog):
        """Test handling of GraphQL errors."""
        import logging
    ...
    @patch('uploader_modules.shopify_api.requests.post')
    def test_fetch_with_status_fn(self, mock_post):
        """Test fetch with status function."""
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
        ...
        result = shopify_api.fetch_all_shopify_categories(api_url, headers, status_fn=mock_status_fn)
        
        assert len(result) == 1
        assert mock_status_fn.call_count >= 1

    @patch('uploader_modules.shopify_api.requests.post')
    def test_fetch_empty_results(self, mock_post):
        """Test fetching when no categories are returned."""
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "data": {
